...
@section('component')
    <div class="container mx-auto px-4 py-8">
        <div class="max-w-5xl mx-auto bg-white rounded-2xl shadow-2xl overflow-hidden border border-gray-100">
                    ...
                    <div class="p-6 space-y-4">
                        ...
                        <div
                            ...
                            <span class="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1">Phone
                                Number</span>
                            <span class="text-base font-semibold text-gray-900">{{ $user->company_phone_number }}</span>
                        </div>
                        <div
                            class="flex flex-col p-4 bg-gray-50 rounded-lg border-l-4 border-purple-500 hover:bg-purple-50 transition-colors duration-200">
                            <span class="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1">Company
            ...
            {{-- Admin Status Change Button - Redesigned --}}
                ...
                <div class="flex justify-center px-8 pb-4">
                    <form id="adminStatusForm" action="{{ route('isadmin', $user->id) }}" method="POST">
                        ...
                        <button type="button"
                            ...
                            class="inline-flex items-center px-6 py-3 rounded-full shadow-lg text-sm font-bold transition-all duration-300
                           {{ $user->is_admin
                               ? 'bg-gradient-to-r from-yellow-400 to-yellow-600 text-white hover:shadow-yellow-200'
                               : 'bg-gradient-to-r from-emerald-400 to-teal-600 text-white hover:shadow-emerald-200' }}">
                            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 mr-2" fill="none" viewBox="0 0 24 24"
                                stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                    d="{{ $user->is_admin
                            ...
                            {{ $user->is_admin ? 'Remove Administrator Access' : 'Grant Administrator Access' }}
                        </button>
                    </form>
                </div>
            @endif

            {{-- Status Change Buttons - Redesigned --}}
                ...
                <div class="flex justify-center gap-4 px-8 pb-8">
                    ...
                    <form action="{{ route('update-status', $user->id) }}" method="POST" class="inline">
                        ...
                        <button type="button"
                            ...
                            class="inline-flex items-center px-6 py-3 rounded-lg text-sm font-bold shadow-lg 
                    ...
                    <form action="{{ route('update-status', $user->id) }}" method="POST" class="inline">
                        ...
                        <button type="button" onclick="confirmStatusChange(this.form, 'Warned', '{{ $user->name }}')"
                            class="inline-flex items-center px-6 py-3 rounded-lg text-sm font-bold shadow-lg 
                            bg-gradient-to-r from-amber-500 to-orange-600 text-white hover:shadow-amber-200
                            transition-all duration-300 transform hover:scale-105">
                            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 mr-2" fill="none"
                                viewBox="0 0 24 24" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
        ...
        <div class="mt-6 text-center text-sm text-gray-500">
            <p>Viewed by {{ auth()->user()->name }} on {{ date('F j, Y, g:i a') }}</p>
        </div>
    </div>
    ...
    {{-- No changes to the existing scripts --}}
    <script>
        function confirmStatusChange(form, status, userName) {
            ...
            const title = isApprove ? 'Approve User?' : 'Set to Pending and Warn?';
            const text = isApprove ?
                `Are you sure you want to approve ${userName}?` :
                `Are you sure you want to set ${userName}'s status to pending and warn them?`;
            ...
            Swal.fire({
                ...
                confirmButtonText: isApprove ? 'Yes, Approve!' : 'Yes, Set to Pending!',
