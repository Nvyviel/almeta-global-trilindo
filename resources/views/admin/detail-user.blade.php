...
@section('component')
    ...
    <div class="mt-4 flex justify-center gap-4 px-8 pb-8">
        {{-- Approve Button --}}
        <form action="{{ route('update-status', $user->id) }}" method="POST" class="inline">
            @csrf
            <input type="hidden" name="status" value="Approved">
            <button type="button" onclick="confirmStatusChange(this.form, 'Approved', '{{ $user->name }}')"
                class="inline-flex items-center px-6 py-2 bg-green-100 text-green-800 rounded-lg text-sm font-medium 
        ...
        <form action="{{ route('update-status', $user->id) }}" method="POST" class="inline">
            ...
            <button type="button" onclick="confirmStatusChange(this.form, 'Warned', '{{ $user->name }}')"
                class="inline-flex items-center px-6 py-2 bg-yellow-100 text-yellow-800 rounded-lg text-sm font-medium 
