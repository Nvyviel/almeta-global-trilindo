...
@section('component')
    <div class="container mx-auto px-4 py-6">
        ...
        <div class="flex justify-between items-center mb-6">
            <h1 class="text-xl font-semibold text-gray-800">Shipping Instructions</h1>
        </div>
            ...
            @forelse($groupedInstructions as $orderId => $instructions)
                <div class="bg-white rounded-lg shadow-sm hover:shadow-md transition-shadow duration-300">
                    <div class="p-6">
                                    ...
                                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                                        <div>
                                            ...
                                            <p class="text-gray-900 font-semibold">
                                                {{ $instruction->container->shipment_container->vessel_name ?? 'No Vessel Name' }}
                                            </p>
                                        </div>
                                        <div>
                                            <span class="text-sm font-medium text-gray-600 block mb-1">Container Type</span>
                                            <p class="text-gray-900 font-semibold">
                                                {{ $instruction->container->container_type ?? 'Unknown' }}
                                            </p>
                                        </div>
                                        <div>
                                            <span class="text-sm font-medium text-gray-600 block mb-1">Created Date</span>
                                            <p class="text-gray-900 font-semibold">
                                                {{ \Carbon\Carbon::parse($instruction->created_at)->format('d M Y') }}
                                            </p>
                                        </div>
                                    </div>
                                </div>
                        ...
                        <form action="{{ route('upload-si-file', $instruction->id) }}" method="POST"
                            enctype="multipart/form-data">
                            @csrf
                            <input type="file" name="si_file" accept="application/pdf" required>
                            <button type="submit">Upload SI File</button>
                        </form>
                        ...
                        <div class="mt-6 flex space-x-3">
                            ...
                            <a href="{{ route('detail-si', $instructions->first()->id) }}" class="flex-1">
                                <button type="button"
                                    class="w-full flex items-center justify-center px-4 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700 transition duration-300">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 mr-2" fill="none"
                                        viewBox="0 0 24 24" stroke="currentColor">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
