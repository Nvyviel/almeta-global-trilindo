...
@section('component')
    <div class="container mx-auto px-2 sm:px-4 py-4 sm:py-6">
        ...
        <div class="flex justify-between items-center mb-4 sm:mb-6">
            <a href="{{ route('history-ro') }}" wire:navigate class="flex items-center text-gray-600 hover:text-gray-800 transition duration-300">
            ...
            <div class="p-3 sm:p-4">
                <form method="GET" action="{{ route('approval-ro') }}" class="space-y-3 sm:space-y-4">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-3 sm:gap-4">
                        <div>
                            <label for="selectedVessel" class="block text-sm font-medium text-gray-600 mb-1">Select Vessel</label>
                            <select id="selectedVessel" name="selectedVessel" class="w-full px-3 sm:px-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 transition duration-300">
                    ...
                    <div class="flex flex-col sm:flex-row space-y-2 sm:space-y-0 sm:space-x-3">
                        <button type="submit" class="w-full sm:flex-1 bg-blue-600 text-white px-4 py-2 rounded-md hover:bg-blue-700 transition duration-300 flex items-center justify-center">
                            <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 sm:h-5 sm:w-5 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        ...
                        <a href="{{ route('approval-ro') }}" wire:navigate class="w-full sm:flex-1 bg-gray-200 text-gray-700 px-4 py-2 rounded-md hover:bg-gray-300 transition duration-300 flex items-center justify-center">
                            <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 sm:h-5 sm:w-5 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    ...
                    <div class="p-4 sm:p-6">
                        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-4">
                            <div class="space-y-2 mb-2 sm:mb-0">
                                <div class="flex items-center">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 sm:h-6 sm:w-6 text-blue-600 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4" />
                                    </svg>
                                    <h2 class="text-lg sm:text-xl font-semibold text-gray-900">{{ $container->user->company_name }}</h2>
                                </div>
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3 sm:gap-4">
                            ...
                            <div class="bg-gray-50 p-3 sm:p-4 rounded-md">
                                ...
                                <p class="text-gray-900 font-semibold">{{ $container->shipment_container->vessel_name }}</p>
                            </div>
                        </div>
                        ...
                        <div class="p-4 sm:p-8">
                            <div class="space-y-2">
                                <div x-data="{ fileChosen: false }" class="space-y-2">
                                    ...
                                    <div class="mt-1 flex flex-col sm:flex-row items-center space-y-2 sm:space-y-0 sm:space-x-4">
                                        <div class="w-full sm:flex-1">
                                            <label class="flex items-center justify-center px-4 sm:px-6 py-2 sm:py-3 border border-gray-300 rounded-lg cursor-pointer hover:bg-gray-50 transition-colors duration-200 @error('pdf_ro') border-red-500 @enderror">
                                    ...
                                    <div x-show="fileChosen" class="text-sm text-gray-500" x-data="{ fileName: '' }" 
                                         x-init="$watch('fileChosen', () => fileName = document.getElementById('pdf_ro').files[0]?.name || '')" 
                                         style="display: none;">
                        ...
                        <div class="mt-4 sm:mt-6 flex flex-col sm:flex-row space-y-2 sm:space-y-0 sm:space-x-3">
                            <form action="{{ route('ro-approved', $container->id) }}" method="POST" class="w-full sm:flex-1" enctype="multipart/form-data">
                                @csrf
                                <input type="file" name="pdf_ro" id="approve_pdf_ro" class="hidden" accept=".pdf">
                                <button type="submit" class="w-full flex items-center justify-center px-4 py-2 bg-green-600 text-white rounded-md hover:bg-green-700 transition duration-300">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 sm:h-5 sm:w-5 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            ...
                            <form action="{{ route('ro-canceled', $container->id) }}" method="POST" class="w-full sm:flex-1">
                                @csrf
                                <button type="submit" class="w-full flex items-center justify-center px-4 py-2 bg-red-600 text-white rounded-md hover:bg-red-700 transition duration-300">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 sm:h-5 sm:w-5 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
